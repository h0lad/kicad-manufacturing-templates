...
Standard 8-layer **1.6 mm** FR-4, **JLC08161H** default stackup on **Nan Ya
NP-155F (TG150)** laminate — 8-layer runs on the high-precision line, not the
TG135 laminate used for 1–6 layer standard. Includes **free ENIG** and **free
via-in-pad (POFV)**. Refer to the
...
Copy the `JLCPCB_8Layer/` directory into your user template path, then
...
| Prepreg        | 1080 x2        | 0.1528 mm  | 4.1  |        |
| In5.Cu         | Cu 0.5 oz      | 0.0152 mm  |      | Signal |
| Core           | FR-4           | 0.3000 mm  | 4.6  |        |
| In6.Cu         | Cu 0.5 oz      | 0.0152 mm  |      | GND    |
| Prepreg        | 2116           | 0.1164 mm  | 4.16 |        |
| B.Cu           | Cu 1 oz        | 0.0350 mm  |      | Signal |

Note the outer prepreg is 2116 (0.1164 mm), not 3313 — outer-layer impedance
geometries differ from the 6-layer template.
...
**Via-in-pad:** POFV (resin-filled, copper-capped) is **free** on 8-layer —
...
Note: outer-layer impedance geometries differ from the 6-layer template due to
2116 outer prepreg (Er 4.16, 0.1164 mm) vs 3313 (Er 4.1, 0.0994 mm). Verify
with a calculator for your design.
