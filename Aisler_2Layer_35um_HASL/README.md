# AISLER 2-Layer 35µm HASL KiCad Template

KiCad project template for **AISLER's 2-layer 35 µm copper, HASL** service.
...
adjusted to the 35 µm **lead-free HASL** rule set. Panasonic laminate, Peters Elpemer
soldermask. HASL gives coarser copper rules than ENIG and requires solder-mask dams
between pads. (July 2025.)

Files are KiCad 8 format; KiCad 10 opens and migrates them on first save.

## Installation

Copy the `Aisler_2Layer_35um_HASL/` directory into your user template path, then
...
2-layer board — single FR-4 core (Panasonic laminate) with 35 µm copper on both sides.
No inner reference planes, no controlled-impedance option.
...
|-------------------------------|----------|----------------------------------|------------|
| Min track width (global)      | 0.20 mm  | min. tracewidth                  | 0.20 mm    |
| Min clearance (global)        | 0.15 mm  | min. spacing copper/copper       | 0.15 mm    |
| Min SMD pad (design)          | 0.20 mm  | min. SMD pad size                | 0.20 mm    |
...
0.20 (HASL min) — 0.25 — 0.3 — 0.4 — 0.5 — 0.8 — 1.0 — 1.5 — 2.0 mm
...
- Default 0.25/0.15 mm, via 0.6/0.3
- Signal_Fine 0.20/0.15 mm (HASL minimum), via 0.45/0.3
...
> HASL vs ENIG on 2-layer: coarser copper (track 0.20 vs 0.125 mm ENIG), larger via
> ring (0.20 vs 0.10 mm), and via drill floor 0.30 vs 0.25 mm. Use ENIG for fine
