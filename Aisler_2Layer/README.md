# AISLER 2-Layer 35µm ENIG KiCad Template

KiCad project template matching **AISLER's 2-layer 35 µm copper, ENIG** design rules.
Based on AISLER's official template ([github.com/AislerHQ/aisler-support](https://github.com/AislerHQ/aisler-support)).
35 µm copper on both sides, **ENIG** finish (Ni 4–7 µm, Au 0.05–0.1 µm), Panasonic
laminate with **Peters Elpemer AS 2467 SM-DG** soldermask (Dk 3.7). Constraints
match the published design-rules page so a design that passes DRC stays within
AISLER's manufacturing capabilities. (July 2025.)

Files are KiCad 8 format; KiCad 10 opens and migrates them on first save.

## Installation

Copy the `Aisler_2Layer/` directory into your user template path, then
